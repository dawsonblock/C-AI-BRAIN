...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores: torch.FloatTensor) -> torch.FloatTensor:
        # Support only single-sequence processing to avoid shape mismatches.
        if isinstance(input_ids, torch.Tensor):
            input_ids = input_ids.tolist()
        if not isinstance(input_ids, list) or (len(input_ids) > 0 and isinstance(input_ids[0], (list, tuple))):
            # Unexpected batched structure; do not modify scores.
            return scores
        ...
        banned_tokens = {ng[-1] for ng in ngrams if ng[:-1] == current_prefix}
